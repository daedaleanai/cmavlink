...
static void append_payload_base(uint8_t **p, struct slugs_message *msg) {
  switch (msg->msg_id) {
  ...
  case SLUGS_MESSAGE_CPU_LOAD:
    append_uint16(p, msg->cpu_load.batVolt);
    append_byte(p, msg->cpu_load.sensLoad);
    append_byte(p, msg->cpu_load.ctrlLoad);
    break;

  case SLUGS_MESSAGE_SENSOR_BIAS:
    append_float32(p, msg->sensor_bias.axBias);
    append_float32(p, msg->sensor_bias.ayBias);
    append_float32(p, msg->sensor_bias.azBias);
    append_float32(p, msg->sensor_bias.gxBias);
    append_float32(p, msg->sensor_bias.gyBias);
    append_float32(p, msg->sensor_bias.gzBias);
    break;

  case SLUGS_MESSAGE_DIAGNOSTIC:
    append_float32(p, msg->diagnostic.diagFl1);
    append_float32(p, msg->diagnostic.diagFl2);
    append_float32(p, msg->diagnostic.diagFl3);
    append_int16(p, msg->diagnostic.diagSh1);
    append_int16(p, msg->diagnostic.diagSh2);
    append_int16(p, msg->diagnostic.diagSh3);
    break;

  case SLUGS_MESSAGE_SLUGS_NAVIGATION:
    append_float32(p, msg->slugs_navigation.u_m);
    append_float32(p, msg->slugs_navigation.phi_c);
    append_float32(p, msg->slugs_navigation.theta_c);
    append_float32(p, msg->slugs_navigation.psiDot_c);
    append_float32(p, msg->slugs_navigation.ay_body);
    append_float32(p, msg->slugs_navigation.totalDist);
    append_float32(p, msg->slugs_navigation.dist2Go);
    append_uint16(p, msg->slugs_navigation.h_c);
    append_byte(p, msg->slugs_navigation.fromWP);
    append_byte(p, msg->slugs_navigation.toWP);
    break;

  case SLUGS_MESSAGE_DATA_LOG:
    append_float32(p, msg->data_log.fl_1);
    append_float32(p, msg->data_log.fl_2);
    append_float32(p, msg->data_log.fl_3);
    append_float32(p, msg->data_log.fl_4);
    append_float32(p, msg->data_log.fl_5);
    append_float32(p, msg->data_log.fl_6);
    break;

  case SLUGS_MESSAGE_GPS_DATE_TIME:
    append_byte(p, msg->gps_date_time.year);
    append_byte(p, msg->gps_date_time.month);
    append_byte(p, msg->gps_date_time.day);
    append_byte(p, msg->gps_date_time.hour);
    append_byte(p, msg->gps_date_time.min);
    append_byte(p, msg->gps_date_time.sec);
    append_byte(p, msg->gps_date_time.clockStat);
    append_byte(p, msg->gps_date_time.visSat);
    append_byte(p, msg->gps_date_time.useSat);
    append_byte(p, msg->gps_date_time.GppGl);
    append_byte(p, msg->gps_date_time.sigUsedMask);
    append_byte(p, msg->gps_date_time.percentUsed);
    break;

  case SLUGS_MESSAGE_MID_LVL_CMDS:
    append_float32(p, msg->mid_lvl_cmds.hCommand);
    append_float32(p, msg->mid_lvl_cmds.uCommand);
    append_float32(p, msg->mid_lvl_cmds.rCommand);
    append_byte(p, msg->mid_lvl_cmds.target);
    break;

  case SLUGS_MESSAGE_CTRL_SRFC_PT:
    append_uint16(p, msg->ctrl_srfc_pt.bitfieldPt);
    append_byte(p, msg->ctrl_srfc_pt.target);
    break;

  case SLUGS_MESSAGE_SLUGS_CAMERA_ORDER:
    append_byte(p, msg->slugs_camera_order.target);
    append_int8(p, msg->slugs_camera_order.pan);
    append_int8(p, msg->slugs_camera_order.tilt);
    append_int8(p, msg->slugs_camera_order.zoom);
    append_int8(p, msg->slugs_camera_order.moveHome);
    break;

  case SLUGS_MESSAGE_CONTROL_SURFACE:
    append_float32(p, msg->control_surface.mControl);
    append_float32(p, msg->control_surface.bControl);
    append_byte(p, msg->control_surface.target);
    append_byte(p, msg->control_surface.idSurface);
    break;

  case SLUGS_MESSAGE_SLUGS_MOBILE_LOCATION:
    append_float32(p, msg->slugs_mobile_location.latitude);
    append_float32(p, msg->slugs_mobile_location.longitude);
    append_byte(p, msg->slugs_mobile_location.target);
    break;

  case SLUGS_MESSAGE_SLUGS_CONFIGURATION_CAMERA:
    append_byte(p, msg->slugs_configuration_camera.target);
    append_byte(p, msg->slugs_configuration_camera.idOrder);
    append_byte(p, msg->slugs_configuration_camera.order);
    break;

  case SLUGS_MESSAGE_ISR_LOCATION:
    append_float32(p, msg->isr_location.latitude);
    append_float32(p, msg->isr_location.longitude);
    append_float32(p, msg->isr_location.height);
    append_byte(p, msg->isr_location.target);
    append_byte(p, msg->isr_location.option1);
    append_byte(p, msg->isr_location.option2);
    append_byte(p, msg->isr_location.option3);
    break;

  case SLUGS_MESSAGE_VOLT_SENSOR:
    append_uint16(p, msg->volt_sensor.voltage);
    append_uint16(p, msg->volt_sensor.reading2);
    append_byte(p, msg->volt_sensor.r2Type);
    break;

  case SLUGS_MESSAGE_PTZ_STATUS:
    append_int16(p, msg->ptz_status.pan);
    append_int16(p, msg->ptz_status.tilt);
    append_byte(p, msg->ptz_status.zoom);
    break;

  case SLUGS_MESSAGE_UAV_STATUS:
    append_float32(p, msg->uav_status.latitude);
    append_float32(p, msg->uav_status.longitude);
    append_float32(p, msg->uav_status.altitude);
    append_float32(p, msg->uav_status.speed);
    append_float32(p, msg->uav_status.course);
    append_byte(p, msg->uav_status.target);
    break;

  case SLUGS_MESSAGE_STATUS_GPS:
    append_float32(p, msg->status_gps.magVar);
    append_uint16(p, msg->status_gps.csFails);
    append_byte(p, msg->status_gps.gpsQuality);
    append_byte(p, msg->status_gps.msgsType);
    append_byte(p, msg->status_gps.posStatus);
    append_int8(p, msg->status_gps.magDir);
    append_byte(p, msg->status_gps.modeInd);
    break;

  case SLUGS_MESSAGE_NOVATEL_DIAG:
    append_uint32(p, msg->novatel_diag.receiverStatus);
    append_float32(p, msg->novatel_diag.posSolAge);
    append_uint16(p, msg->novatel_diag.csFails);
    append_byte(p, msg->novatel_diag.timeStatus);
    append_byte(p, msg->novatel_diag.solStatus);
    append_byte(p, msg->novatel_diag.posType);
    append_byte(p, msg->novatel_diag.velType);
    break;

  case SLUGS_MESSAGE_SENSOR_DIAG:
    append_float32(p, msg->sensor_diag.float1);
    append_float32(p, msg->sensor_diag.float2);
    append_int16(p, msg->sensor_diag.int1);
    append_int8(p, msg->sensor_diag.char1);
    break;

  case SLUGS_MESSAGE_BOOT:
    append_uint32(p, msg->boot.version);
    break;
...
static void append_payload_ext(uint8_t **p, struct slugs_message *msg) {
  switch (msg->msg_id) {
...
size_t slugs_message_serialize_v1(struct slugs_message *msg, uint8_t *buf,
  ...
  *p++ = msg->msg_id;
  ...
  crc = x25(crc, slugs_crcextra(msg->msg_id));
