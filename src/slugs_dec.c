...
static void read_payload_base(const uint8_t **p, struct slugs_message *msg) {
  switch (msg->msg_id) {
  ...
  case SLUGS_MESSAGE_CPU_LOAD:
    msg->cpu_load.batVolt = read_uint16(p);
    msg->cpu_load.sensLoad = read_byte(p);
    msg->cpu_load.ctrlLoad = read_byte(p);
    break;

  case SLUGS_MESSAGE_SENSOR_BIAS:
    msg->sensor_bias.axBias = read_float32(p);
    msg->sensor_bias.ayBias = read_float32(p);
    msg->sensor_bias.azBias = read_float32(p);
    msg->sensor_bias.gxBias = read_float32(p);
    msg->sensor_bias.gyBias = read_float32(p);
    msg->sensor_bias.gzBias = read_float32(p);
    break;

  case SLUGS_MESSAGE_DIAGNOSTIC:
    msg->diagnostic.diagFl1 = read_float32(p);
    msg->diagnostic.diagFl2 = read_float32(p);
    msg->diagnostic.diagFl3 = read_float32(p);
    msg->diagnostic.diagSh1 = read_int16(p);
    msg->diagnostic.diagSh2 = read_int16(p);
    msg->diagnostic.diagSh3 = read_int16(p);
    break;

  case SLUGS_MESSAGE_SLUGS_NAVIGATION:
    msg->slugs_navigation.u_m = read_float32(p);
    msg->slugs_navigation.phi_c = read_float32(p);
    msg->slugs_navigation.theta_c = read_float32(p);
    msg->slugs_navigation.psiDot_c = read_float32(p);
    msg->slugs_navigation.ay_body = read_float32(p);
    msg->slugs_navigation.totalDist = read_float32(p);
    msg->slugs_navigation.dist2Go = read_float32(p);
    msg->slugs_navigation.h_c = read_uint16(p);
    msg->slugs_navigation.fromWP = read_byte(p);
    msg->slugs_navigation.toWP = read_byte(p);
    break;

  case SLUGS_MESSAGE_DATA_LOG:
    msg->data_log.fl_1 = read_float32(p);
    msg->data_log.fl_2 = read_float32(p);
    msg->data_log.fl_3 = read_float32(p);
    msg->data_log.fl_4 = read_float32(p);
    msg->data_log.fl_5 = read_float32(p);
    msg->data_log.fl_6 = read_float32(p);
    break;

  case SLUGS_MESSAGE_GPS_DATE_TIME:
    msg->gps_date_time.year = read_byte(p);
    msg->gps_date_time.month = read_byte(p);
    msg->gps_date_time.day = read_byte(p);
    msg->gps_date_time.hour = read_byte(p);
    msg->gps_date_time.min = read_byte(p);
    msg->gps_date_time.sec = read_byte(p);
    msg->gps_date_time.clockStat = read_byte(p);
    msg->gps_date_time.visSat = read_byte(p);
    msg->gps_date_time.useSat = read_byte(p);
    msg->gps_date_time.GppGl = read_byte(p);
    msg->gps_date_time.sigUsedMask = read_byte(p);
    msg->gps_date_time.percentUsed = read_byte(p);
    break;

  case SLUGS_MESSAGE_MID_LVL_CMDS:
    msg->mid_lvl_cmds.hCommand = read_float32(p);
    msg->mid_lvl_cmds.uCommand = read_float32(p);
    msg->mid_lvl_cmds.rCommand = read_float32(p);
    msg->mid_lvl_cmds.target = read_byte(p);
    break;

  case SLUGS_MESSAGE_CTRL_SRFC_PT:
    msg->ctrl_srfc_pt.bitfieldPt = read_uint16(p);
    msg->ctrl_srfc_pt.target = read_byte(p);
    break;

  case SLUGS_MESSAGE_SLUGS_CAMERA_ORDER:
    msg->slugs_camera_order.target = read_byte(p);
    msg->slugs_camera_order.pan = read_int8(p);
    msg->slugs_camera_order.tilt = read_int8(p);
    msg->slugs_camera_order.zoom = read_int8(p);
    msg->slugs_camera_order.moveHome = read_int8(p);
    break;

  case SLUGS_MESSAGE_CONTROL_SURFACE:
    msg->control_surface.mControl = read_float32(p);
    msg->control_surface.bControl = read_float32(p);
    msg->control_surface.target = read_byte(p);
    msg->control_surface.idSurface = read_byte(p);
    break;

  case SLUGS_MESSAGE_SLUGS_MOBILE_LOCATION:
    msg->slugs_mobile_location.latitude = read_float32(p);
    msg->slugs_mobile_location.longitude = read_float32(p);
    msg->slugs_mobile_location.target = read_byte(p);
    break;

  case SLUGS_MESSAGE_SLUGS_CONFIGURATION_CAMERA:
    msg->slugs_configuration_camera.target = read_byte(p);
    msg->slugs_configuration_camera.idOrder = read_byte(p);
    msg->slugs_configuration_camera.order = read_byte(p);
    break;

  case SLUGS_MESSAGE_ISR_LOCATION:
    msg->isr_location.latitude = read_float32(p);
    msg->isr_location.longitude = read_float32(p);
    msg->isr_location.height = read_float32(p);
    msg->isr_location.target = read_byte(p);
    msg->isr_location.option1 = read_byte(p);
    msg->isr_location.option2 = read_byte(p);
    msg->isr_location.option3 = read_byte(p);
    break;

  case SLUGS_MESSAGE_VOLT_SENSOR:
    msg->volt_sensor.voltage = read_uint16(p);
    msg->volt_sensor.reading2 = read_uint16(p);
    msg->volt_sensor.r2Type = read_byte(p);
    break;

  case SLUGS_MESSAGE_PTZ_STATUS:
    msg->ptz_status.pan = read_int16(p);
    msg->ptz_status.tilt = read_int16(p);
    msg->ptz_status.zoom = read_byte(p);
    break;

  case SLUGS_MESSAGE_UAV_STATUS:
    msg->uav_status.latitude = read_float32(p);
    msg->uav_status.longitude = read_float32(p);
    msg->uav_status.altitude = read_float32(p);
    msg->uav_status.speed = read_float32(p);
    msg->uav_status.course = read_float32(p);
    msg->uav_status.target = read_byte(p);
    break;

  case SLUGS_MESSAGE_STATUS_GPS:
    msg->status_gps.magVar = read_float32(p);
    msg->status_gps.csFails = read_uint16(p);
    msg->status_gps.gpsQuality = read_byte(p);
    msg->status_gps.msgsType = read_byte(p);
    msg->status_gps.posStatus = read_byte(p);
    msg->status_gps.magDir = read_int8(p);
    msg->status_gps.modeInd = read_byte(p);
    break;

  case SLUGS_MESSAGE_NOVATEL_DIAG:
    msg->novatel_diag.receiverStatus = read_uint32(p);
    msg->novatel_diag.posSolAge = read_float32(p);
    msg->novatel_diag.csFails = read_uint16(p);
    msg->novatel_diag.timeStatus = read_byte(p);
    msg->novatel_diag.solStatus = read_byte(p);
    msg->novatel_diag.posType = read_byte(p);
    msg->novatel_diag.velType = read_byte(p);
    break;

  case SLUGS_MESSAGE_SENSOR_DIAG:
    msg->sensor_diag.float1 = read_float32(p);
    msg->sensor_diag.float2 = read_float32(p);
    msg->sensor_diag.int1 = read_int16(p);
    msg->sensor_diag.char1 = read_int8(p);
    break;

  case SLUGS_MESSAGE_BOOT:
    msg->boot.version = read_uint32(p);
    break;
...
static void read_payload_ext(const uint8_t **p, struct slugs_message *msg) {
  switch (msg->msg_id) {
