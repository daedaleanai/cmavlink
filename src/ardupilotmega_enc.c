...
static void append_payload_base(uint8_t **p,
  ...
  case ARDUPILOTMEGA_MESSAGE_SENSOR_OFFSETS:
    append_float32(p, msg->sensor_offsets.mag_declination);
    append_int32(p, msg->sensor_offsets.raw_press);
    append_int32(p, msg->sensor_offsets.raw_temp);
    append_float32(p, msg->sensor_offsets.gyro_cal_x);
    append_float32(p, msg->sensor_offsets.gyro_cal_y);
    append_float32(p, msg->sensor_offsets.gyro_cal_z);
    append_float32(p, msg->sensor_offsets.accel_cal_x);
    append_float32(p, msg->sensor_offsets.accel_cal_y);
    append_float32(p, msg->sensor_offsets.accel_cal_z);
    append_int16(p, msg->sensor_offsets.mag_ofs_x);
    append_int16(p, msg->sensor_offsets.mag_ofs_y);
    append_int16(p, msg->sensor_offsets.mag_ofs_z);
    break;

  case ARDUPILOTMEGA_MESSAGE_SET_MAG_OFFSETS:
    append_int16(p, msg->set_mag_offsets.mag_ofs_x);
    append_int16(p, msg->set_mag_offsets.mag_ofs_y);
    append_int16(p, msg->set_mag_offsets.mag_ofs_z);
    append_byte(p, msg->set_mag_offsets.target_system);
    append_byte(p, msg->set_mag_offsets.target_component);
    break;

  case ARDUPILOTMEGA_MESSAGE_MEMINFO:
    append_uint16(p, msg->meminfo.brkval);
    append_uint16(p, msg->meminfo.freemem);
    break;

  case ARDUPILOTMEGA_MESSAGE_AP_ADC:
    append_uint16(p, msg->ap_adc.adc1);
    append_uint16(p, msg->ap_adc.adc2);
    append_uint16(p, msg->ap_adc.adc3);
    append_uint16(p, msg->ap_adc.adc4);
    append_uint16(p, msg->ap_adc.adc5);
    append_uint16(p, msg->ap_adc.adc6);
    break;

  case ARDUPILOTMEGA_MESSAGE_DIGICAM_CONFIGURE:
    append_float32(p, msg->digicam_configure.extra_value);
    append_uint16(p, msg->digicam_configure.shutter_speed);
    append_byte(p, msg->digicam_configure.target_system);
    append_byte(p, msg->digicam_configure.target_component);
    append_byte(p, msg->digicam_configure.mode);
    append_byte(p, msg->digicam_configure.aperture);
    append_byte(p, msg->digicam_configure.iso);
    append_byte(p, msg->digicam_configure.exposure_type);
    append_byte(p, msg->digicam_configure.command_id);
    append_byte(p, msg->digicam_configure.engine_cut_off);
    append_byte(p, msg->digicam_configure.extra_param);
    break;

  case ARDUPILOTMEGA_MESSAGE_DIGICAM_CONTROL:
    append_float32(p, msg->digicam_control.extra_value);
    append_byte(p, msg->digicam_control.target_system);
    append_byte(p, msg->digicam_control.target_component);
    append_byte(p, msg->digicam_control.session);
    append_byte(p, msg->digicam_control.zoom_pos);
    append_int8(p, msg->digicam_control.zoom_step);
    append_byte(p, msg->digicam_control.focus_lock);
    append_byte(p, msg->digicam_control.shot);
    append_byte(p, msg->digicam_control.command_id);
    append_byte(p, msg->digicam_control.extra_param);
    break;

  case ARDUPILOTMEGA_MESSAGE_MOUNT_CONFIGURE:
    append_byte(p, msg->mount_configure.target_system);
    append_byte(p, msg->mount_configure.target_component);
    append_byte(p, msg->mount_configure.mount_mode);
    append_byte(p, msg->mount_configure.stab_roll);
    append_byte(p, msg->mount_configure.stab_pitch);
    append_byte(p, msg->mount_configure.stab_yaw);
    break;

  case ARDUPILOTMEGA_MESSAGE_MOUNT_CONTROL:
    append_int32(p, msg->mount_control.input_a);
    append_int32(p, msg->mount_control.input_b);
    append_int32(p, msg->mount_control.input_c);
    append_byte(p, msg->mount_control.target_system);
    append_byte(p, msg->mount_control.target_component);
    append_byte(p, msg->mount_control.save_position);
    break;

  case ARDUPILOTMEGA_MESSAGE_MOUNT_STATUS:
    append_int32(p, msg->mount_status.pointing_a);
    append_int32(p, msg->mount_status.pointing_b);
    append_int32(p, msg->mount_status.pointing_c);
    append_byte(p, msg->mount_status.target_system);
    append_byte(p, msg->mount_status.target_component);
    break;

  case ARDUPILOTMEGA_MESSAGE_FENCE_POINT:
    append_float32(p, msg->fence_point.lat);
    append_float32(p, msg->fence_point.lng);
    append_byte(p, msg->fence_point.target_system);
    append_byte(p, msg->fence_point.target_component);
    append_byte(p, msg->fence_point.idx);
    append_byte(p, msg->fence_point.count);
    break;

  case ARDUPILOTMEGA_MESSAGE_FENCE_FETCH_POINT:
    append_byte(p, msg->fence_fetch_point.target_system);
    append_byte(p, msg->fence_fetch_point.target_component);
    append_byte(p, msg->fence_fetch_point.idx);
    break;

  case ARDUPILOTMEGA_MESSAGE_FENCE_STATUS:
    append_uint32(p, msg->fence_status.breach_time);
    append_uint16(p, msg->fence_status.breach_count);
    append_byte(p, msg->fence_status.breach_status);
    append_byte(p, msg->fence_status.breach_type);
    break;

  case ARDUPILOTMEGA_MESSAGE_AHRS:
    append_float32(p, msg->ahrs.omegaIx);
    append_float32(p, msg->ahrs.omegaIy);
    append_float32(p, msg->ahrs.omegaIz);
    append_float32(p, msg->ahrs.accel_weight);
    append_float32(p, msg->ahrs.renorm_val);
    append_float32(p, msg->ahrs.error_rp);
    append_float32(p, msg->ahrs.error_yaw);
    break;

  case ARDUPILOTMEGA_MESSAGE_SIMSTATE:
    append_float32(p, msg->simstate.roll);
    append_float32(p, msg->simstate.pitch);
    append_float32(p, msg->simstate.yaw);
    append_float32(p, msg->simstate.xacc);
    append_float32(p, msg->simstate.yacc);
    append_float32(p, msg->simstate.zacc);
    append_float32(p, msg->simstate.xgyro);
    append_float32(p, msg->simstate.ygyro);
    append_float32(p, msg->simstate.zgyro);
    append_int32(p, msg->simstate.lat);
    append_int32(p, msg->simstate.lng);
    break;

  case ARDUPILOTMEGA_MESSAGE_HWSTATUS:
    append_uint16(p, msg->hwstatus.Vcc);
    append_byte(p, msg->hwstatus.I2Cerr);
    break;

  case ARDUPILOTMEGA_MESSAGE_RADIO:
    append_uint16(p, msg->radio.rxerrors);
    append_uint16(p, msg->radio.fixed);
    append_byte(p, msg->radio.rssi);
    append_byte(p, msg->radio.remrssi);
    append_byte(p, msg->radio.txbuf);
    append_byte(p, msg->radio.noise);
    append_byte(p, msg->radio.remnoise);
    break;

  case ARDUPILOTMEGA_MESSAGE_LIMITS_STATUS:
    append_uint32(p, msg->limits_status.last_trigger);
    append_uint32(p, msg->limits_status.last_action);
    append_uint32(p, msg->limits_status.last_recovery);
    append_uint32(p, msg->limits_status.last_clear);
    append_uint16(p, msg->limits_status.breach_count);
    append_byte(p, msg->limits_status.limits_state);
    append_byte(p, msg->limits_status.mods_enabled);
    append_byte(p, msg->limits_status.mods_required);
    append_byte(p, msg->limits_status.mods_triggered);
    break;

  case ARDUPILOTMEGA_MESSAGE_WIND:
    append_float32(p, msg->wind.direction);
    append_float32(p, msg->wind.speed);
    append_float32(p, msg->wind.speed_z);
    break;

  case ARDUPILOTMEGA_MESSAGE_DATA16:
    append_byte(p, msg->data16.type);
    append_byte(p, msg->data16.len);
    for (size_t i = 0; i < 16; i++)
      append_byte(p, msg->data16.data[i]);
    break;

  case ARDUPILOTMEGA_MESSAGE_DATA32:
    append_byte(p, msg->data32.type);
    append_byte(p, msg->data32.len);
    for (size_t i = 0; i < 32; i++)
      append_byte(p, msg->data32.data[i]);
    break;

  case ARDUPILOTMEGA_MESSAGE_DATA64:
    append_byte(p, msg->data64.type);
    append_byte(p, msg->data64.len);
    for (size_t i = 0; i < 64; i++)
      append_byte(p, msg->data64.data[i]);
    break;

  case ARDUPILOTMEGA_MESSAGE_DATA96:
    append_byte(p, msg->data96.type);
    append_byte(p, msg->data96.len);
    for (size_t i = 0; i < 96; i++)
      append_byte(p, msg->data96.data[i]);
    break;

  case ARDUPILOTMEGA_MESSAGE_RANGEFINDER:
    append_float32(p, msg->rangefinder.distance);
    append_float32(p, msg->rangefinder.voltage);
    break;

  case ARDUPILOTMEGA_MESSAGE_AIRSPEED_AUTOCAL:
    append_float32(p, msg->airspeed_autocal.vx);
    append_float32(p, msg->airspeed_autocal.vy);
    append_float32(p, msg->airspeed_autocal.vz);
    append_float32(p, msg->airspeed_autocal.diff_pressure);
    append_float32(p, msg->airspeed_autocal.EAS2TAS);
    append_float32(p, msg->airspeed_autocal.ratio);
    append_float32(p, msg->airspeed_autocal.state_x);
    append_float32(p, msg->airspeed_autocal.state_y);
    append_float32(p, msg->airspeed_autocal.state_z);
    append_float32(p, msg->airspeed_autocal.Pax);
    append_float32(p, msg->airspeed_autocal.Pby);
    append_float32(p, msg->airspeed_autocal.Pcz);
    break;

  case ARDUPILOTMEGA_MESSAGE_RALLY_POINT:
    append_int32(p, msg->rally_point.lat);
    append_int32(p, msg->rally_point.lng);
    append_int16(p, msg->rally_point.alt);
    append_int16(p, msg->rally_point.break_alt);
    append_uint16(p, msg->rally_point.land_dir);
    append_byte(p, msg->rally_point.target_system);
    append_byte(p, msg->rally_point.target_component);
    append_byte(p, msg->rally_point.idx);
    append_byte(p, msg->rally_point.count);
    append_byte(p, msg->rally_point.flags);
    break;

  case ARDUPILOTMEGA_MESSAGE_RALLY_FETCH_POINT:
    append_byte(p, msg->rally_fetch_point.target_system);
    append_byte(p, msg->rally_fetch_point.target_component);
    append_byte(p, msg->rally_fetch_point.idx);
    break;

  case ARDUPILOTMEGA_MESSAGE_COMPASSMOT_STATUS:
    append_float32(p, msg->compassmot_status.current);
    append_float32(p, msg->compassmot_status.CompensationX);
    append_float32(p, msg->compassmot_status.CompensationY);
    append_float32(p, msg->compassmot_status.CompensationZ);
    append_uint16(p, msg->compassmot_status.throttle);
    append_uint16(p, msg->compassmot_status.interference);
    break;

  case ARDUPILOTMEGA_MESSAGE_AHRS2:
    append_float32(p, msg->ahrs2.roll);
    append_float32(p, msg->ahrs2.pitch);
    append_float32(p, msg->ahrs2.yaw);
    append_float32(p, msg->ahrs2.altitude);
    append_int32(p, msg->ahrs2.lat);
    append_int32(p, msg->ahrs2.lng);
    break;

  case ARDUPILOTMEGA_MESSAGE_CAMERA_STATUS:
    append_uint64(p, msg->camera_status.time_usec);
    append_float32(p, msg->camera_status.p1);
    append_float32(p, msg->camera_status.p2);
    append_float32(p, msg->camera_status.p3);
    append_float32(p, msg->camera_status.p4);
    append_uint16(p, msg->camera_status.img_idx);
    append_byte(p, msg->camera_status.target_system);
    append_byte(p, msg->camera_status.cam_idx);
    append_byte(p, msg->camera_status.event_id);
    break;

  case ARDUPILOTMEGA_MESSAGE_CAMERA_FEEDBACK:
    append_uint64(p, msg->camera_feedback.time_usec);
    append_int32(p, msg->camera_feedback.lat);
    append_int32(p, msg->camera_feedback.lng);
    append_float32(p, msg->camera_feedback.alt_msl);
    append_float32(p, msg->camera_feedback.alt_rel);
    append_float32(p, msg->camera_feedback.roll);
    append_float32(p, msg->camera_feedback.pitch);
    append_float32(p, msg->camera_feedback.yaw);
    append_float32(p, msg->camera_feedback.foc_len);
    append_uint16(p, msg->camera_feedback.img_idx);
    append_byte(p, msg->camera_feedback.target_system);
    append_byte(p, msg->camera_feedback.cam_idx);
    append_byte(p, msg->camera_feedback.flags);
    break;

  case ARDUPILOTMEGA_MESSAGE_BATTERY2:
    append_uint16(p, msg->battery2.voltage);
    append_int16(p, msg->battery2.current_battery);
    break;

  case ARDUPILOTMEGA_MESSAGE_AHRS3:
    append_float32(p, msg->ahrs3.roll);
    append_float32(p, msg->ahrs3.pitch);
    append_float32(p, msg->ahrs3.yaw);
    append_float32(p, msg->ahrs3.altitude);
    append_int32(p, msg->ahrs3.lat);
    append_int32(p, msg->ahrs3.lng);
    append_float32(p, msg->ahrs3.v1);
    append_float32(p, msg->ahrs3.v2);
    append_float32(p, msg->ahrs3.v3);
    append_float32(p, msg->ahrs3.v4);
    break;

  case ARDUPILOTMEGA_MESSAGE_AUTOPILOT_VERSION_REQUEST:
    append_byte(p, msg->autopilot_version_request.target_system);
    append_byte(p, msg->autopilot_version_request.target_component);
    break;

  case ARDUPILOTMEGA_MESSAGE_REMOTE_LOG_DATA_BLOCK:
    append_uint32(p, msg->remote_log_data_block.seqno);
    append_byte(p, msg->remote_log_data_block.target_system);
    append_byte(p, msg->remote_log_data_block.target_component);
    for (size_t i = 0; i < 200; i++)
      append_byte(p, msg->remote_log_data_block.data[i]);
    break;

  case ARDUPILOTMEGA_MESSAGE_REMOTE_LOG_BLOCK_STATUS:
    append_uint32(p, msg->remote_log_block_status.seqno);
    append_byte(p, msg->remote_log_block_status.target_system);
    append_byte(p, msg->remote_log_block_status.target_component);
    append_byte(p, msg->remote_log_block_status.status);
    break;

  case ARDUPILOTMEGA_MESSAGE_LED_CONTROL:
    append_byte(p, msg->led_control.target_system);
    append_byte(p, msg->led_control.target_component);
    append_byte(p, msg->led_control.instance);
    append_byte(p, msg->led_control.pattern);
    append_byte(p, msg->led_control.custom_len);
    for (size_t i = 0; i < 24; i++)
      append_byte(p, msg->led_control.custom_bytes[i]);
    break;

  case ARDUPILOTMEGA_MESSAGE_MAG_CAL_PROGRESS:
    append_float32(p, msg->mag_cal_progress.direction_x);
    append_float32(p, msg->mag_cal_progress.direction_y);
    append_float32(p, msg->mag_cal_progress.direction_z);
    append_byte(p, msg->mag_cal_progress.compass_id);
    append_byte(p, msg->mag_cal_progress.cal_mask);
    append_byte(p, msg->mag_cal_progress.cal_status);
    append_byte(p, msg->mag_cal_progress.attempt);
    append_byte(p, msg->mag_cal_progress.completion_pct);
    for (size_t i = 0; i < 10; i++)
      append_byte(p, msg->mag_cal_progress.completion_mask[i]);
    break;

  case ARDUPILOTMEGA_MESSAGE_MAG_CAL_REPORT:
    append_float32(p, msg->mag_cal_report.fitness);
    append_float32(p, msg->mag_cal_report.ofs_x);
    append_float32(p, msg->mag_cal_report.ofs_y);
    append_float32(p, msg->mag_cal_report.ofs_z);
    append_float32(p, msg->mag_cal_report.diag_x);
    append_float32(p, msg->mag_cal_report.diag_y);
    append_float32(p, msg->mag_cal_report.diag_z);
    append_float32(p, msg->mag_cal_report.offdiag_x);
    append_float32(p, msg->mag_cal_report.offdiag_y);
    append_float32(p, msg->mag_cal_report.offdiag_z);
    append_byte(p, msg->mag_cal_report.compass_id);
    append_byte(p, msg->mag_cal_report.cal_mask);
    append_byte(p, msg->mag_cal_report.cal_status);
    append_byte(p, msg->mag_cal_report.autosaved);
    break;

  case ARDUPILOTMEGA_MESSAGE_EKF_STATUS_REPORT:
    append_float32(p, msg->ekf_status_report.velocity_variance);
    append_float32(p, msg->ekf_status_report.pos_horiz_variance);
    append_float32(p, msg->ekf_status_report.pos_vert_variance);
    append_float32(p, msg->ekf_status_report.compass_variance);
    append_float32(p, msg->ekf_status_report.terrain_alt_variance);
    append_uint16(p, msg->ekf_status_report.flags);
    break;

  case ARDUPILOTMEGA_MESSAGE_PID_TUNING:
    append_float32(p, msg->pid_tuning.desired);
    append_float32(p, msg->pid_tuning.achieved);
    append_float32(p, msg->pid_tuning.FF);
    append_float32(p, msg->pid_tuning.P);
    append_float32(p, msg->pid_tuning.I);
    append_float32(p, msg->pid_tuning.D);
    append_byte(p, msg->pid_tuning.axis);
    break;

  case ARDUPILOTMEGA_MESSAGE_DEEPSTALL:
    append_int32(p, msg->deepstall.landing_lat);
    append_int32(p, msg->deepstall.landing_lon);
    append_int32(p, msg->deepstall.path_lat);
    append_int32(p, msg->deepstall.path_lon);
    append_int32(p, msg->deepstall.arc_entry_lat);
    append_int32(p, msg->deepstall.arc_entry_lon);
    append_float32(p, msg->deepstall.altitude);
    append_float32(p, msg->deepstall.expected_travel_distance);
    append_float32(p, msg->deepstall.cross_track_error);
    append_byte(p, msg->deepstall.stage);
    break;

  case ARDUPILOTMEGA_MESSAGE_GIMBAL_REPORT:
    append_float32(p, msg->gimbal_report.delta_time);
    append_float32(p, msg->gimbal_report.delta_angle_x);
    append_float32(p, msg->gimbal_report.delta_angle_y);
    append_float32(p, msg->gimbal_report.delta_angle_z);
    append_float32(p, msg->gimbal_report.delta_velocity_x);
    append_float32(p, msg->gimbal_report.delta_velocity_y);
    append_float32(p, msg->gimbal_report.delta_velocity_z);
    append_float32(p, msg->gimbal_report.joint_roll);
    append_float32(p, msg->gimbal_report.joint_el);
    append_float32(p, msg->gimbal_report.joint_az);
    append_byte(p, msg->gimbal_report.target_system);
    append_byte(p, msg->gimbal_report.target_component);
    break;

  case ARDUPILOTMEGA_MESSAGE_GIMBAL_CONTROL:
    append_float32(p, msg->gimbal_control.demanded_rate_x);
    append_float32(p, msg->gimbal_control.demanded_rate_y);
    append_float32(p, msg->gimbal_control.demanded_rate_z);
    append_byte(p, msg->gimbal_control.target_system);
    append_byte(p, msg->gimbal_control.target_component);
    break;

  case ARDUPILOTMEGA_MESSAGE_GIMBAL_TORQUE_CMD_REPORT:
    append_int16(p, msg->gimbal_torque_cmd_report.rl_torque_cmd);
    append_int16(p, msg->gimbal_torque_cmd_report.el_torque_cmd);
    append_int16(p, msg->gimbal_torque_cmd_report.az_torque_cmd);
    append_byte(p, msg->gimbal_torque_cmd_report.target_system);
    append_byte(p, msg->gimbal_torque_cmd_report.target_component);
    break;

  case ARDUPILOTMEGA_MESSAGE_GOPRO_HEARTBEAT:
    append_byte(p, msg->gopro_heartbeat.status);
    append_byte(p, msg->gopro_heartbeat.capture_mode);
    append_byte(p, msg->gopro_heartbeat.flags);
    break;

  case ARDUPILOTMEGA_MESSAGE_GOPRO_GET_REQUEST:
    append_byte(p, msg->gopro_get_request.target_system);
    append_byte(p, msg->gopro_get_request.target_component);
    append_byte(p, msg->gopro_get_request.cmd_id);
    break;

  case ARDUPILOTMEGA_MESSAGE_GOPRO_GET_RESPONSE:
    append_byte(p, msg->gopro_get_response.cmd_id);
    append_byte(p, msg->gopro_get_response.status);
    for (size_t i = 0; i < 4; i++)
      append_byte(p, msg->gopro_get_response.value[i]);
    break;

  case ARDUPILOTMEGA_MESSAGE_GOPRO_SET_REQUEST:
    append_byte(p, msg->gopro_set_request.target_system);
    append_byte(p, msg->gopro_set_request.target_component);
    append_byte(p, msg->gopro_set_request.cmd_id);
    for (size_t i = 0; i < 4; i++)
      append_byte(p, msg->gopro_set_request.value[i]);
    break;

  case ARDUPILOTMEGA_MESSAGE_GOPRO_SET_RESPONSE:
    append_byte(p, msg->gopro_set_response.cmd_id);
    append_byte(p, msg->gopro_set_response.status);
    break;

  case ARDUPILOTMEGA_MESSAGE_RPM:
    append_float32(p, msg->rpm.rpm1);
    append_float32(p, msg->rpm.rpm2);
    break;
  ...
  case ARDUPILOTMEGA_MESSAGE_UAVIONIX_ADSB_OUT_CFG:
    append_uint32(p, msg->uavionix_adsb_out_cfg.ICAO);
    append_uint16(p, msg->uavionix_adsb_out_cfg.stallSpeed);
    for (size_t i = 0; i < 9; i++)
      append_byte(p, msg->uavionix_adsb_out_cfg.callsign[i]);
    append_byte(p, msg->uavionix_adsb_out_cfg.emitterType);
    append_byte(p, msg->uavionix_adsb_out_cfg.aircraftSize);
    append_byte(p, msg->uavionix_adsb_out_cfg.gpsOffsetLat);
    append_byte(p, msg->uavionix_adsb_out_cfg.gpsOffsetLon);
    append_byte(p, msg->uavionix_adsb_out_cfg.rfSelect);
    break;

  case ARDUPILOTMEGA_MESSAGE_UAVIONIX_ADSB_OUT_DYNAMIC:
    append_uint32(p, msg->uavionix_adsb_out_dynamic.utcTime);
    append_int32(p, msg->uavionix_adsb_out_dynamic.gpsLat);
    append_int32(p, msg->uavionix_adsb_out_dynamic.gpsLon);
    append_int32(p, msg->uavionix_adsb_out_dynamic.gpsAlt);
    append_int32(p, msg->uavionix_adsb_out_dynamic.baroAltMSL);
    append_uint32(p, msg->uavionix_adsb_out_dynamic.accuracyHor);
    append_uint16(p, msg->uavionix_adsb_out_dynamic.accuracyVert);
    append_uint16(p, msg->uavionix_adsb_out_dynamic.accuracyVel);
    append_int16(p, msg->uavionix_adsb_out_dynamic.velVert);
    append_int16(p, msg->uavionix_adsb_out_dynamic.velNS);
    append_int16(p, msg->uavionix_adsb_out_dynamic.VelEW);
    append_uint16(p, msg->uavionix_adsb_out_dynamic.state);
    append_uint16(p, msg->uavionix_adsb_out_dynamic.squawk);
    append_byte(p, msg->uavionix_adsb_out_dynamic.gpsFix);
    append_byte(p, msg->uavionix_adsb_out_dynamic.numSats);
    append_byte(p, msg->uavionix_adsb_out_dynamic.emergencyStatus);
    break;

  case ARDUPILOTMEGA_MESSAGE_UAVIONIX_ADSB_TRANSCEIVER_HEALTH_REPORT:
    append_byte(p, msg->uavionix_adsb_transceiver_health_report.rfHealth);
    break;

  case ARDUPILOTMEGA_MESSAGE_DEVICE_OP_READ:
    append_uint32(p, msg->device_op_read.request_id);
    append_byte(p, msg->device_op_read.target_system);
    append_byte(p, msg->device_op_read.target_component);
    append_byte(p, msg->device_op_read.bustype);
    append_byte(p, msg->device_op_read.bus);
    append_byte(p, msg->device_op_read.address);
    for (size_t i = 0; i < 40; i++)
      append_byte(p, msg->device_op_read.busname[i]);
    append_byte(p, msg->device_op_read.regstart);
    append_byte(p, msg->device_op_read.count);
    break;

  case ARDUPILOTMEGA_MESSAGE_DEVICE_OP_READ_REPLY:
    append_uint32(p, msg->device_op_read_reply.request_id);
    append_byte(p, msg->device_op_read_reply.result);
    append_byte(p, msg->device_op_read_reply.regstart);
    append_byte(p, msg->device_op_read_reply.count);
    for (size_t i = 0; i < 128; i++)
      append_byte(p, msg->device_op_read_reply.data[i]);
    break;

  case ARDUPILOTMEGA_MESSAGE_DEVICE_OP_WRITE:
    append_uint32(p, msg->device_op_write.request_id);
    append_byte(p, msg->device_op_write.target_system);
    append_byte(p, msg->device_op_write.target_component);
    append_byte(p, msg->device_op_write.bustype);
    append_byte(p, msg->device_op_write.bus);
    append_byte(p, msg->device_op_write.address);
    for (size_t i = 0; i < 40; i++)
      append_byte(p, msg->device_op_write.busname[i]);
    append_byte(p, msg->device_op_write.regstart);
    append_byte(p, msg->device_op_write.count);
    for (size_t i = 0; i < 128; i++)
      append_byte(p, msg->device_op_write.data[i]);
    break;

  case ARDUPILOTMEGA_MESSAGE_DEVICE_OP_WRITE_REPLY:
    append_uint32(p, msg->device_op_write_reply.request_id);
    append_byte(p, msg->device_op_write_reply.result);
    break;

  case ARDUPILOTMEGA_MESSAGE_ADAP_TUNING:
    append_float32(p, msg->adap_tuning.desired);
    append_float32(p, msg->adap_tuning.achieved);
    append_float32(p, msg->adap_tuning.error);
    append_float32(p, msg->adap_tuning.theta);
    append_float32(p, msg->adap_tuning.omega);
    append_float32(p, msg->adap_tuning.sigma);
    append_float32(p, msg->adap_tuning.theta_dot);
    append_float32(p, msg->adap_tuning.omega_dot);
    append_float32(p, msg->adap_tuning.sigma_dot);
    append_float32(p, msg->adap_tuning.f);
    append_float32(p, msg->adap_tuning.f_dot);
    append_float32(p, msg->adap_tuning.u);
    append_byte(p, msg->adap_tuning.axis);
    break;

  case ARDUPILOTMEGA_MESSAGE_VISION_POSITION_DELTA:
    append_uint64(p, msg->vision_position_delta.time_usec);
    append_uint64(p, msg->vision_position_delta.time_delta_usec);
    for (size_t i = 0; i < 3; i++)
      append_float32(p, msg->vision_position_delta.angle_delta[i]);
    for (size_t i = 0; i < 3; i++)
      append_float32(p, msg->vision_position_delta.position_delta[i]);
    append_float32(p, msg->vision_position_delta.confidence);
    break;

  case ARDUPILOTMEGA_MESSAGE_AOA_SSA:
    append_uint64(p, msg->aoa_ssa.time_usec);
    append_float32(p, msg->aoa_ssa.AOA);
    append_float32(p, msg->aoa_ssa.SSA);
    break;

  case ARDUPILOTMEGA_MESSAGE_ESC_TELEMETRY_1_TO_4:

    for (size_t i = 0; i < 4; i++)
      append_uint16(p, msg->esc_telemetry_1_to_4.voltage[i]);
    for (size_t i = 0; i < 4; i++)
      append_uint16(p, msg->esc_telemetry_1_to_4.current[i]);
    for (size_t i = 0; i < 4; i++)
      append_uint16(p, msg->esc_telemetry_1_to_4.totalcurrent[i]);
    for (size_t i = 0; i < 4; i++)
      append_uint16(p, msg->esc_telemetry_1_to_4.rpm[i]);
    for (size_t i = 0; i < 4; i++)
      append_uint16(p, msg->esc_telemetry_1_to_4.count[i]);
    for (size_t i = 0; i < 4; i++)
      append_byte(p, msg->esc_telemetry_1_to_4.temperature[i]);
    break;

  case ARDUPILOTMEGA_MESSAGE_ESC_TELEMETRY_5_TO_8:

    for (size_t i = 0; i < 4; i++)
      append_uint16(p, msg->esc_telemetry_5_to_8.voltage[i]);
    for (size_t i = 0; i < 4; i++)
      append_uint16(p, msg->esc_telemetry_5_to_8.current[i]);
    for (size_t i = 0; i < 4; i++)
      append_uint16(p, msg->esc_telemetry_5_to_8.totalcurrent[i]);
    for (size_t i = 0; i < 4; i++)
      append_uint16(p, msg->esc_telemetry_5_to_8.rpm[i]);
    for (size_t i = 0; i < 4; i++)
      append_uint16(p, msg->esc_telemetry_5_to_8.count[i]);
    for (size_t i = 0; i < 4; i++)
      append_byte(p, msg->esc_telemetry_5_to_8.temperature[i]);
    break;

  case ARDUPILOTMEGA_MESSAGE_ESC_TELEMETRY_9_TO_12:

    for (size_t i = 0; i < 4; i++)
      append_uint16(p, msg->esc_telemetry_9_to_12.voltage[i]);
    for (size_t i = 0; i < 4; i++)
      append_uint16(p, msg->esc_telemetry_9_to_12.current[i]);
    for (size_t i = 0; i < 4; i++)
      append_uint16(p, msg->esc_telemetry_9_to_12.totalcurrent[i]);
    for (size_t i = 0; i < 4; i++)
      append_uint16(p, msg->esc_telemetry_9_to_12.rpm[i]);
    for (size_t i = 0; i < 4; i++)
      append_uint16(p, msg->esc_telemetry_9_to_12.count[i]);
    for (size_t i = 0; i < 4; i++)
      append_byte(p, msg->esc_telemetry_9_to_12.temperature[i]);
    break;
...
static void append_payload_ext(uint8_t **p, struct ardupilotmega_message *msg) {
  switch (msg->msg_id) {
  ...
  case ARDUPILOTMEGA_MESSAGE_MEMINFO:
    append_uint32(p, msg->meminfo.freemem32);
    break;

  case ARDUPILOTMEGA_MESSAGE_CAMERA_FEEDBACK:
    append_uint16(p, msg->camera_feedback.completed_captures);
    break;

  case ARDUPILOTMEGA_MESSAGE_MAG_CAL_REPORT:
    append_float32(p, msg->mag_cal_report.orientation_confidence);
    append_byte(p, msg->mag_cal_report.old_orientation);
    append_byte(p, msg->mag_cal_report.new_orientation);
    break;

  case ARDUPILOTMEGA_MESSAGE_EKF_STATUS_REPORT:
    append_float32(p, msg->ekf_status_report.airspeed_variance);
    break;
