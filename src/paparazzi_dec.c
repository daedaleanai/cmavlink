...
static void read_payload_base(const uint8_t **p,
  ...
  case PAPARAZZI_MESSAGE_SCRIPT_ITEM:
    msg->script_item.seq = read_uint16(p);
    msg->script_item.target_system = read_byte(p);
    msg->script_item.target_component = read_byte(p);
    for (size_t i = 0; i < 50; i++)
      msg->script_item.name[i] = read_byte(p);
    break;

  case PAPARAZZI_MESSAGE_SCRIPT_REQUEST:
    msg->script_request.seq = read_uint16(p);
    msg->script_request.target_system = read_byte(p);
    msg->script_request.target_component = read_byte(p);
    break;

  case PAPARAZZI_MESSAGE_SCRIPT_REQUEST_LIST:
    msg->script_request_list.target_system = read_byte(p);
    msg->script_request_list.target_component = read_byte(p);
    break;

  case PAPARAZZI_MESSAGE_SCRIPT_COUNT:
    msg->script_count.count = read_uint16(p);
    msg->script_count.target_system = read_byte(p);
    msg->script_count.target_component = read_byte(p);
    break;

  case PAPARAZZI_MESSAGE_SCRIPT_CURRENT:
    msg->script_current.seq = read_uint16(p);
    break;
...
static void read_payload_ext(const uint8_t **p, struct paparazzi_message *msg) {
  switch (msg->msg_id) {
