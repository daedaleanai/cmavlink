...
static void read_payload_base(const uint8_t **p, struct ualberta_message *msg) {
  switch (msg->msg_id) {
  ...
  case UALBERTA_MESSAGE_NAV_FILTER_BIAS:
    msg->nav_filter_bias.usec = read_uint64(p);
    msg->nav_filter_bias.accel_0 = read_float32(p);
    msg->nav_filter_bias.accel_1 = read_float32(p);
    msg->nav_filter_bias.accel_2 = read_float32(p);
    msg->nav_filter_bias.gyro_0 = read_float32(p);
    msg->nav_filter_bias.gyro_1 = read_float32(p);
    msg->nav_filter_bias.gyro_2 = read_float32(p);
    break;

  case UALBERTA_MESSAGE_RADIO_CALIBRATION:

    for (size_t i = 0; i < 3; i++)
      msg->radio_calibration.aileron[i] = read_uint16(p);
    for (size_t i = 0; i < 3; i++)
      msg->radio_calibration.elevator[i] = read_uint16(p);
    for (size_t i = 0; i < 3; i++)
      msg->radio_calibration.rudder[i] = read_uint16(p);
    for (size_t i = 0; i < 2; i++)
      msg->radio_calibration.gyro[i] = read_uint16(p);
    for (size_t i = 0; i < 5; i++)
      msg->radio_calibration.pitch[i] = read_uint16(p);
    for (size_t i = 0; i < 5; i++)
      msg->radio_calibration.throttle[i] = read_uint16(p);
    break;

  case UALBERTA_MESSAGE_UALBERTA_SYS_STATUS:
    msg->ualberta_sys_status.mode = read_byte(p);
    msg->ualberta_sys_status.nav_mode = read_byte(p);
    msg->ualberta_sys_status.pilot = read_byte(p);
    break;
...
static void read_payload_ext(const uint8_t **p, struct ualberta_message *msg) {
  switch (msg->msg_id) {
