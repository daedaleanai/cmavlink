...
static void append_payload_base(uint8_t **p, struct ualberta_message *msg) {
  switch (msg->msg_id) {
  ...
  case UALBERTA_MESSAGE_NAV_FILTER_BIAS:
    append_uint64(p, msg->nav_filter_bias.usec);
    append_float32(p, msg->nav_filter_bias.accel_0);
    append_float32(p, msg->nav_filter_bias.accel_1);
    append_float32(p, msg->nav_filter_bias.accel_2);
    append_float32(p, msg->nav_filter_bias.gyro_0);
    append_float32(p, msg->nav_filter_bias.gyro_1);
    append_float32(p, msg->nav_filter_bias.gyro_2);
    break;

  case UALBERTA_MESSAGE_RADIO_CALIBRATION:

    for (size_t i = 0; i < 3; i++)
      append_uint16(p, msg->radio_calibration.aileron[i]);
    for (size_t i = 0; i < 3; i++)
      append_uint16(p, msg->radio_calibration.elevator[i]);
    for (size_t i = 0; i < 3; i++)
      append_uint16(p, msg->radio_calibration.rudder[i]);
    for (size_t i = 0; i < 2; i++)
      append_uint16(p, msg->radio_calibration.gyro[i]);
    for (size_t i = 0; i < 5; i++)
      append_uint16(p, msg->radio_calibration.pitch[i]);
    for (size_t i = 0; i < 5; i++)
      append_uint16(p, msg->radio_calibration.throttle[i]);
    break;

  case UALBERTA_MESSAGE_UALBERTA_SYS_STATUS:
    append_byte(p, msg->ualberta_sys_status.mode);
    append_byte(p, msg->ualberta_sys_status.nav_mode);
    append_byte(p, msg->ualberta_sys_status.pilot);
    break;
...
static void append_payload_ext(uint8_t **p, struct ualberta_message *msg) {
  switch (msg->msg_id) {
...
size_t ualberta_message_serialize_v1(struct ualberta_message *msg, uint8_t *buf,
  ...
  *p++ = msg->msg_id;
  ...
  crc = x25(crc, ualberta_crcextra(msg->msg_id));
